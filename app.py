...
def register():
    ...
    if email in users:
        if users[email]['is_verified']:
            ...
            send_email_api(email, "إعادة إرسال: تأكيد حسابك في متجر غرسة 🌿", f"مرحباً {users[email]['username']}،\n\nيرجى الضغط على الرابط التالي لتفعيل حسابك:\n\n{link}")
    ...
    token = s.dumps(email, salt='email-confirm-salt')
    link = url_for('verify_account', token=token, _external=True)
    send_email_api(email, "تأكيد حسابك في متجر غرسة 🌿", f"مرحباً {username}،\n\nيرجى تفعيل حسابك عبر الرابط الآمن التالي:\n\n{link}")
...
def forgot_password():
    if request.method == 'POST':
        ...
        if email in users:
            ...
            send_email_api(email, "استعادة كلمة المرور - متجر غرسة 🌿", f"مرحباً،\n\nلقد طلبت استعادة كلمة المرور لحسابك في متجر غرسة.\nيرجى الضغط على الرابط التالي لتعيين كلمة مرور جديدة (الرابط صالح لمدة 10 دقائق):\n\n{link}")
